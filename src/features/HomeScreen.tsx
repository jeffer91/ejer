...
export function HomeScreen({ userId, onNavigate }: Props) {
  ...
  const reload = useCallback(async () => {
    ...
    setWater(await listRows<HydrationLog>('hydration_logs', userId));
    setSessions(await listRows<WorkoutSession>('workout_sessions', userId));
  }, [userId]);
  ...
  const waterToday = useMemo(
    () => water
      .filter((item) => localDateKey(new Date(item.logged_at)) === today)
  ...
  const workoutToday = useMemo(
    () => sessions.find((item) => localDateKey(new Date(item.completed_at)) === today),
    [sessions, today],
  );
