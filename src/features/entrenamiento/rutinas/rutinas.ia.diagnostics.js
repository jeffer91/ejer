/*
  ...
  Función o funciones:
    ...
    - Validar que el parser detecte rutina, días, bloques, cardio, fútbol y formulario.
...
equipo=mancuernas, máquinas y balón
dias=2
...
- ejercicio=Sentadilla goblet | tipo=fuerza | series=3 | repeticiones=10 | descanso=60s | duracion= | intensidad=media | notas=técnica controlada
- ejercicio=Peso muerto rumano | tipo=fuerza | series=3 | repeticiones=10 | descanso=60s | duracion= | intensidad=media | notas=espalda neutra
[FIN_BLOQUE]

[BLOQUE]
tipo=cardio
nombre=Cardio final
- ejercicio=Caminata inclinada | tipo=cardio | series= | repeticiones= | descanso= | duracion=12min | intensidad=suave | notas=ritmo cómodo
[FIN_BLOQUE]
[FIN_DIA]
...
- ejercicio=Conducción de balón | tipo=futbol | series=4 | repeticiones= | descanso=45s | duracion=3min | intensidad=media | notas=mantener control
- ejercicio=Pases contra pared | tipo=tecnica | series=3 | repeticiones=20 | descanso=45s | duracion= | intensidad=media | notas=usar ambos pies
[FIN_BLOQUE]

[BLOQUE]
tipo=movilidad
nombre=Movilidad final
- ejercicio=Estiramiento de cadera | tipo=movilidad | series=1 | repeticiones= | descanso= | duracion=6min | intensidad=suave | notas=sin dolor
[FIN_BLOQUE]
[FIN_DIA]
...
export function ejecutarDiagnosticoRutinaIA({ interpretarTextoRutinaIA, puedeGuardar = false } = {}) {
  ...
  const pasos = [
    ...
    crearPasoDiagnostico("Bloques detectados", Number(resumen.bloques || 0) >= 4, `${resumen.bloques || 0} bloque(s)`),
...
function crearVistaDiagnostico(diagnostico = {}) {
  ...
  caja.appendChild(crearElemento("span", "", `${resumen.dias || 0} día(s) · ${resumen.bloques || 0} bloque(s) · ${resumen.ejercicios || 0} ejercicio(s)`));
...
export function insertarPanelDiagnosticoRutinaIA(pantalla, { onEjecutar } = {}) {
  ...
  textoTop.appendChild(crearElemento("p", "", "Prueba el contrato, parser, bloques, cardio, fútbol, formulario y guardado sin crear una rutina de prueba."));
