/*
  ...
  Función:
    ...
    - Preparar la sincronización entre datos locales y Firebase.

  Se conecta con:
    - src/firebase/firebase.app.js
    - src/firebase/firebase.config.js
    - src/app.js cuando se active la sincronización final.
    - src/estadisticas/estadisticas.service.js cuando se cree.
...
export async function guardarResumenEstadisticasFirestore(
  resumen,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
  return guardarDocumentoFijo(
    usuarioId,
    COLECCIONES.ESTADISTICAS,
    DOCUMENTOS.RESUMEN,
    resumen
  );
}
...
export async function guardarEntrenamientoFirestore(
  entrenamiento,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
  return crearDocumentoEnSubcoleccion(
    usuarioId,
    COLECCIONES.ENTRENAMIENTOS,
    entrenamiento
  );
}

export async function listarEntrenamientosFirestore(
  cantidad = 50,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
  return listarSubcoleccionOrdenada(
    usuarioId,
    COLECCIONES.ENTRENAMIENTOS,
    cantidad
  );
...
export async function listarPesosFirestore(
  cantidad = 50,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
...
export async function guardarRecomendacionFirestore(
  recomendacion,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
  return crearDocumentoEnSubcoleccion(
    usuarioId,
    COLECCIONES.RECOMENDACIONES,
    recomendacion
  );
}

export async function listarRecomendacionesFirestore(
  cantidad = 20,
  usuarioId = obtenerUsuarioIdPorDefecto()
) {
  return listarSubcoleccionOrdenada(
...
async function listarSubcoleccionOrdenada(usuarioId, nombreColeccion, cantidad) {
  const db = obtenerFirestore();
  const ref = collection(db, COLECCIONES.USUARIOS, usuarioId, nombreColeccion);
  const consulta = query(ref, orderBy("creadoEn", "desc"), limit(cantidad));
...
function prepararPayload(data) {
  return JSON.parse(
    JSON.stringify(data, (_clave, valor) => {
      if (valor === undefined) {
        return null;
      }
