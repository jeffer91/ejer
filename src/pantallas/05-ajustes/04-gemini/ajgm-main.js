...
Función o funciones:
- Cargar y guardar configuración local de Gemini.
- Probar conexión real con Gemini Interactions API.
- Generar recomendaciones reales usando servicios compartidos.
Con qué se conecta:
- ajgm-index.html
- ajgm.css
- data/ajgm-demo-data.json
- ../../../shared/integraciones/gemini/fit-gm-prompts.js
- ../../../shared/integraciones/gemini/fit-gm-client.service.js
- ../../../shared/integraciones/gemini/fit-gm-recommendations.service.js
========================================================= */
(function(){
  'use strict';
  ...
  function conectarEventos(){
    ...
    el.btnGenerar.addEventListener('click',generarRecomendacionReal);
  ...
  function llenarFormulario(){var c=estado.config;el.inputApiKey.value=c.apiKey;el.inputModelo.value=c.modelo;el.inputTemperatura.value=c.temperatura;el.inputMaxTokens.value=c.maxTokens;el.inputEstado.value=c.estado;el.inputPrompt.value=estado.prompts.length?estado.prompts[0].texto:'';el.inputNota.value=c.nota;}
  ...
  function validar(config){if(!config.apiKey||config.apiKey.indexOf('PEGAR_')===0){mostrarMensaje('Primero pega una API Key real.',true);return false;}if(!config.modelo||config.modelo.indexOf('PEGAR_')===0){mostrarMensaje('Primero escribe el modelo que vas a usar.',true);return false;}if(!el.inputPrompt.value.trim()){mostrarMensaje('El prompt base no puede quedar vacío.',true);return false;}if(config.temperatura<0||config.temperatura>1){mostrarMensaje('La temperatura debe estar entre 0 y 1.',true);return false;}if(config.maxTokens<200||config.maxTokens>8000){mostrarMensaje('El máximo de tokens debe estar entre 200 y 8000.',true);return false;}return true;}
  function guardarConfiguracion(){var config=leerFormulario();if(!validar(config)){return;}estado.config=config;actualizarPromptBase();guardarLocal();mostrarMensaje('Configuración Gemini guardada.',false);renderizarTodo();}
  function probarConfiguracionLocal(){var config=leerFormulario();if(!validar(config)){return;}config.estado='Configurado';config.ultimaPrueba=fechaHoraActual();estado.config=config;el.inputEstado.value='Configurado';actualizarPromptBase();guardarLocal();mostrarMensaje('Validación local correcta.',false);renderizarTodo();}
  function probarConexionReal(){var config=leerFormulario();if(!validar(config)){return;}guardarConfiguracion();bloquear(true,'Probando conexión con Gemini...');window.FitGeminiClient.ping(config).then(function(res){config.estado='Probado';config.ultimaPrueba=fechaHoraActual();estado.config=config;el.inputEstado.value='Probado';guardarLocal();mostrarMensaje('Conexión Gemini correcta: '+(res.message||'OK'),false);renderizarTodo();}).catch(function(error){mostrarMensaje('No se pudo conectar con Gemini: '+error.message,true);}).finally(function(){bloquear(false);});}
  function generarRecomendacionReal(){var config=leerFormulario();if(!validar(config)){return;}guardarConfiguracion();if(!window.FitGeminiRecommendations){mostrarMensaje('Servicio de recomendaciones Gemini no cargado.',true);return;}bloquear(true,'Generando recomendación real...');window.FitGeminiRecommendations.generate().then(function(res){mostrarMensaje('Recomendación generada y guardada. Revisa localStorage fitness-jeff-gemini-ultima-respuesta.',false);}).catch(function(error){mostrarMensaje('No se pudo generar: '+error.message,true);}).finally(function(){bloquear(false);});}
  function actualizarPromptBase(){var texto=el.inputPrompt.value.trim();if(!estado.prompts.length){estado.prompts.push({nombre:'Sistema general',texto:texto});return;}estado.prompts[0].texto=texto;}
  function renderizarTodo(){renderizarResumen();renderizarPrompts();}
  function renderizarResumen(){var c=estado.config;el.resumenEstado.textContent=c.estado;el.resumenModelo.textContent=acortar(c.modelo);el.resumenTemp.textContent=String(c.temperatura);el.resumenPrueba.textContent=c.ultimaPrueba||'Sin probar';}
  function renderizarPrompts(){el.prompts.innerHTML='';estado.prompts.forEach(function(prompt,index){var div=document.createElement('div');div.className='ajgm-prompt-item';div.innerHTML='<span>Prompt '+(index+1)+'</span><strong>'+esc(prompt.nombre)+'</strong><p>'+esc(prompt.texto)+'</p>';el.prompts.appendChild(div);});}
  ...
  function bloquear(active,text){[el.btnConexion,el.btnGenerar].forEach(function(btn){btn.disabled=active;});if(text){mostrarMensaje(text,false);}}
